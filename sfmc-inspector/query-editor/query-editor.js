/**
 * SFMC Inspector Reloaded - Query Editor MVP.
...
(function () {
    ...
    document.body.innerHTML = [
      '<main class="feature-unavailable">',
        '<div class="feature-unavailable-card">',
          ...
          "<h1>Query Editor is not available yet</h1>",
  ...
  if (!FeatureFlags.require("queryEditor", { homePath: "panel/popup.html" })) return;
  ...
  function renderNotes(safetyWarnings) {
    var notes = safetyWarnings || [];
    if (!notes.length) {
      return '<div class="note-list"><div class="note"><div class="note-title">' +
        '<span class="tag tag--green">OK</span>No run-plan notes</div><p>The query is clean enough for a preview run plan.</p></div></div>';
    }

    return '<div class="note-list">' + notes.map(function (d) {
      var cls = d.severity === "error" ? "red" : d.severity === "warning" ? "yellow" : "blue";
      return '<div class="note">' +
        '<div class="note-title"><span class="tag tag--' + cls + '">' + escHtml(d.severity || "info") + "</span>" +
          escHtml(d.title || "Run-plan note") + "</div>" +
  ...
  function renderAnalysis(buildPlan) {
    ...
    els.analysisPanel.innerHTML =
      '<div class="analysis-summary">' +
        ...
        '</div></div>' +
        '<div class="analysis-score">' + (hasErrors ? "Check" : "OK") + "</div>" +
      "</div>" +
      renderNotes(safetyWarnings);
    ...
    if (columns.length) {
      ...
      els.consoleBox.textContent = "Paste SQL to see inferred output columns and the SFMC API workflow this MVP would run.";
    }
    ...
    if (hasErrors) planErrors.push("Resolve blocking SQL notes.");
  ...
  els.btnInsertSample.addEventListener("click", function () {
    ...
    if (!els.targetName.value.trim()) els.targetName.value = "SFMC Inspector Reloaded Preview";
